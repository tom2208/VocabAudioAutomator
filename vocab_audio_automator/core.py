...
def build_prompts(vocab_to_process, global_words_string, config, status_callback=print):
    ...
    if global_words_string:
        global_text = config["prompts"]["global_words_addon"].format(global_words=global_words_string)
    ...
    for row in vocab_to_process:
        ...
        if bonus_words:
            ...
            if mode == "all":
                local_text = config["prompts"]["bonus_words_all"].format(extra_words=bonus_words)
            elif mode == "some":
...
def generate_audio_gpt4o(client, text, filename, config):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Repeat this text exactly word-for-word: {text}"},
        ],
    )
    ...
    try:
        ...
        for i, (target, source) in enumerate(results):
            ...
            if audio_model == "openai":
                voice = generate_audio_gpt4o(clients["openai"], target, file_path, config)
